    ...
    <section className={styles.gridSection}>
      <div className={styles.container}>
        ...
        <p className={styles.description}>
          Successful cryptocurrency exchanges are very profitable for their owners, even if they are not very large.
          Exchanges make money on trading commissions. Launch your NFT or Crypto Exchange platform without investing much time and hard earned money.
        </p>
        <div className={styles.grid}>
          {platforms.map((platform, index) => (
            <div key={index} className={styles.gridItem}>
              <img src={platform.image} alt={platform.name} className={styles.icon} />
              <p>{platform.name}</p>
            </div>
          ))}
        </div>
